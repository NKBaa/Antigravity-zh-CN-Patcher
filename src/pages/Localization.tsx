...
type UpdateState = 'idle' | 'checking' | 'current' | 'available' | 'downloading' | 'ready' | 'manual' | 'unsupported' | 'error';
...
const Localization = () => {
    ...
    const installUpdate = async () => {
        ...
        try {
            ...
            try {
                await invoke('open_update_download');
                setUpdateError('');
                setUpdateState('manual');
                showToast(c.manualDownload, 'warning', 6000);
                return;
            } catch (_) {
                // Keep the updater error if the download page cannot be opened.
            }
            const message = String(error);
            setUpdateError(message);
            setUpdateState('error');
        ...
        <div data-page-scroll className="h-full overflow-y-auto px-5 py-4 lg:px-7 lg:py-5 max-w-7xl mx-auto w-full space-y-4">
            ...
            <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-base-200 dark:bg-base-100">
                ...
                <div className="mt-4 border-t border-slate-200 pt-4 dark:border-white/10">
                    ...
                    {updateState === 'manual' && <p className="text-sm text-amber-600 dark:text-amber-300">{c.manualDownload}</p>}
