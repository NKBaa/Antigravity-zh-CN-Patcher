...
pub fn get_tray_texts(lang: &str) -> TrayTexts {
    ...
    let (open_default, restart_default) = match lang {
        "zh" | "zh-CN" | "zh-Hans" => ("打开目标", "重启目标"),
        "zh-TW" | "zh-Hant" => ("開啟目標", "重啟目標"),
        _ => ("Open Target", "Restart Target"),
    };
    ...
    TrayTexts {
        ...
        restart_target: t
            .get("restart_target")
            .cloned()
            .unwrap_or_else(|| restart_default.to_string()),
        quit: t
            .get("quit")
            .cloned()
