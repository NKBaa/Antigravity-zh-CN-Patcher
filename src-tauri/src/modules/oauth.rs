...
// Google OAuth configuration is supplied at runtime. Never commit client
// credentials to the repository; the Patcher does not require OAuth by default.
...
fn build_registry() -> OAuthClientRegistry {
    ...
    // OAuth is optional. A built-in client is only registered when both
    // values are explicitly provided by the host environment.
    if let (Ok(client_id), Ok(client_secret)) = (
        std::env::var("ANTIGRAVITY_GOOGLE_CLIENT_ID"),
        std::env::var("ANTIGRAVITY_GOOGLE_CLIENT_SECRET"),
    ) {
