...
const GITHUB_RAW_URL: &str =
    "https://raw.githubusercontent.com/NKBaa/Antigravity-zh-CN-Patcher/main/package.json";
const JSDELIVR_URL: &str =
    "https://cdn.jsdelivr.net/gh/NKBaa/Antigravity-zh-CN-Patcher@main/package.json";
const CURRENT_VERSION: &str = env!("CARGO_PKG_VERSION");
const DEFAULT_CHECK_INTERVAL_HOURS: u64 = 24;
const UPDATE_TEMP_CLEANUP_AGE: Duration = Duration::from_secs(10 * 60);
const UPDATE_TEMP_PREFIXES: [&str; 2] = ["Antigravity Patcher-", "antigravity-tools-"];
...
const UPDATER_JSON_URL: &str =
    "https://github.com/NKBaa/Antigravity-zh-CN-Patcher/releases/latest/download/updater.json";
...
async fn check_updater_json() -> Result<UpdateInfo, String> {
    ...
    let download_url = format!(
        "https://github.com/NKBaa/Antigravity-zh-CN-Patcher/releases/tag/v{}",
        latest_version
    );
...
async fn check_static_url(url: &str, source_name: &str) -> Result<UpdateInfo, String> {
    ...
    let download_url = "https://github.com/NKBaa/Antigravity-zh-CN-Patcher/releases/latest".to_string();
...
/// Remove stale temporary artifacts left by a previous Windows updater run.
///
/// The updater stores its installer below the system temp directory. The
/// installer may still be locked while the replacement process is starting,
/// so cleanup is deliberately retried on the next application launch and only
/// targets the updater's own, recognisable prefixes.
pub fn cleanup_stale_update_artifacts() -> usize {
    let temp_dir = std::env::temp_dir();
    let cutoff = SystemTime::now()
        .checked_sub(UPDATE_TEMP_CLEANUP_AGE)
    ...
    for entry in entries.flatten() {
        ...
        let is_updater_artifact = UPDATE_TEMP_PREFIXES.iter().any(|prefix| {
            name.starts_with(prefix) && (name.contains("-installer") || name.contains("-updater-"))
        });
        ...
        match result {
            Ok(()) => {
                removed += 1;
                logger::log_info(&format!("Removed stale updater artifact: {}", path.display()));
            }
            Err(error) => logger::log_warn(&format!(
                "Updater artifact is still in use, retrying later '{}': {}",
                path.display(),
                error
            )),
        }
