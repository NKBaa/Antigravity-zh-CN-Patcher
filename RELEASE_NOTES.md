...
- 补充 Antigravity 2.12.2 的 IDE 安装提示、浏览器设置迁移通知、应用版本及工件审核说明。
- 修复“未找到项目”被拆分翻译的问题，并补充智能体操作与工具权限设置说明。
- 增加启动更新检查、镜像切换、下载百分比、解压和重启的全过程提示。
- 根据完整菜单语境，将项目选择器空状态优化为“暂无项目”。
- 调整项目选择器空状态字号，使其与紧凑菜单布局协调。
- 为“暂无项目”增加与菜单内容一致的留白。
- 修复更新后从临时目录启动新版可能触发 `0xc0000142` 的问题，改用稳定版本目录并增加启动确认与重试。
- 自动更新的 ZIP 与带版本号的新程序改为保存在原程序所在文件夹。
- 补充登录欢迎页、身份验证、企业帐户及引导导航按钮的中文翻译。
- 补充地区不可用提示、账号资格说明及切换账号按钮的中文翻译。
- 补充 Google、Google Cloud、SSO、WIF 配置、远程控制反馈类别等登录与反馈页面翻译。
- 修复登录提示正文、欢迎标题及 FAQ 链接被拆分为多个文本节点时无法翻译的问题。
- 增加登录页标题的容器级翻译，修复组件包装导致标题仍显示英文的问题。
